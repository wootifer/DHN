...
a = np.loadtxt(Config_test.DATA_SAVE_PATH + Config_test.DATA_FILENAME, dtype=np.float32)
a = list(a)
...
gar_number = 0
false_number = 0
upper = max(false_list) + 0.1
x_ = np.linspace(0, upper, 200)
for i in x_:
    # for k in true_list:
    #     if k > i:
...
x1 = np.linspace(0, upper, 200)
ys1, bias = np.histogram(true_list, bins=200, range=(0, upper))
y1 = ys1 / len(true_list) * 100

ys2, bias = np.histogram(false_list, bins=200, range=(0, upper))
y2 = ys2 / len(false_list) * 100

plt.plot(x1, y1, label='Genuine', linewidth=1, color='green')
plt.plot(x1, y2, label='Imposer', linewidth=1, color='red', linestyle='--')
